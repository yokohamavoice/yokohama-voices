...
- 全体分析は毎日午前3時（日本時間、CronはUTCの `0 18 * * *`）に更新します。前回のPCA軸・グループ分けを初期値に使い、全対象の回答を読み直して更新します。データ・設定が同じなら計算を省略します。
- 群の領域は日次の点から凸包と表示上の余白で描き、件数を併記します。個々の点は切り替え表示します。領域の準備と背景描画は日次結果の取得・表示切り替え時に行い、回答ごとには本人の重ね描きだけを更新します。
- 完了済みの地図・投影モデルをR2で共有し、回答を保存した直後はブラウザー内で本人だけを投影します。本人の仮の所属は日次の群人数・共通点の集計に加えません。新しい意見は次の日次モデルから反映します。
- `/api/analysis` と `/api/community?analysis=1` は保存済みの結果を読むだけです。前者に既知の `snapshot` を渡すと、変更がない場合はモデルを再送せず、小さい応答だけを返します。内部のセッションID対応は送りません。
- 日次ジョブが失敗しても前回の地図を表示します。ただし掲載判断・分析設定が変わった場合は古い結果を表示せず、次の日次更新を待ちます。同日の重複実行は日付と期限付きロックで抑止します。
...
`0005_daily_analysis.sql` を含む未適用migrationを配置前に適用してください。`worker.ts` が通常の画面配信とscheduledイベントを受け、Cron設定はビルド済みのWorker設定にも引き継ぎます。初回は次の午前3時に集計します。初回や分析設定の変更直後に有効な保存版がない場合は、回答の受付を続けながら集計待ちを表示します。GitHubへのpushだけではこのプロジェクトの配置は実行されません。

ローカルで翌朝を待たずに検証する場合は、ビルド後の `node tests/run-isolated.mjs .` を使います。専用の一時DB/R2で初回・翌日・同日重複・掲載判断後のscheduledイベントを実行します。本番のHTTP APIに全体計算を起動する入口は設けていません。
...
node --experimental-strip-types tests/daily-projection.test.ts
